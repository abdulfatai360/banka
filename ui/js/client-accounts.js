...
const accountsDetailsTemplate = document.querySelector(
  '#accts-details-template'
).textContent;
...
const renderAccountsDetails = accounts => {
  ...
  accounts.forEach(account => {
    accountsDetailsHtml += generateAccountsDetailsHtml(
      accountsDetailsTemplate,
      account
    );
  });
...
const fetchClientAccounts = () => {
  ...
  fetch(url, init)
    ...
    .then(response => {
      ...
      document
        .querySelector('.accts-details__inner')
...
const resetAccountOpeningForm = () => {
  ...
  accountOpeningInputs.forEach(input =>
...
const toggleAccountOpeningFormDisplay = () => {
  ...
  if (
    !accountOpeningFormContainer.classList.contains('acct-opn-form-visible')
  ) {
    window.scrollTo({
      top: 0,
      behavior: 'smooth'
...
class validateAccountOpeningInputs {
  static openingBalance(errors) {
    const errorsListElem = document.querySelector(
      '.opening-balance-field .validation-error-list'
    );
  ...
  static accountType(errors) {
    const errorsListElem = document.querySelector(
      '.account-type-field .validation-error-list'
    );
    errorsListElem.textContent = '';
